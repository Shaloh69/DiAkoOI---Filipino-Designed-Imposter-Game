...
export default defineConfig({
  ...
  integrations: [
    ...
    // Starlight 0.41 removed `routeBasePath`: the URL now follows the content
    // directory, so src/content/docs/changelog/** serves at /changelog/**.
    // Its `[...slug]` catch-all sits behind AstroWind's explicit routes, and
    // the 404 route is left to AstroWind's own 404.astro.
    //
    // Must precede mdx(): Starlight pulls in astro-expressive-code, which
    // requires that ordering to process code blocks on MDX pages.
    // AstroWind stays the landing page and Starlight owns /changelog only:
    // Starlight is excellent for structured content and poor as a landing
    // page, so each is used for what it is good at (docs/07-TEMPLATES.md §2).
    starlight({
      title: 'DiAkoOi Changelog',
      description: 'Release notes for DiAkoOi.',
      disable404Route: true,
      sidebar: [
        { label: 'Releases', items: [{ autogenerate: { directory: 'changelog' } }] },
      ],
    }),
    mdx(),
